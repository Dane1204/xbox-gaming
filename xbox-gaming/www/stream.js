...
var app = {
    ...
    // Function to retrieve the list of consoles
    getConsoles: function() {
        return new Promise(function(resolve, reject) {
            fetch('/api/consoles').then(response => {
                if (response.status !== 200) {
                    reject({ error: 'xCloudPlayer Client - /api/consoles status code is not 200' });
                } else {
    ...
    startSession(type, serverId) {
        ...
        return new Promise((resolve, reject) => {
            if (type === 'xhome') {
                            ...
                            client.createOffer().then((offer) => {
                                ...
                                this.isExchangeReady('/api/config').then((data) => {
                                    this.isExchangeReady('/api/config/sdp').then((data) => {
                                        ...
                                        // ICE has been set, let's do ICE
                                        this.isExchangeReady('/api/config/ice').then((data) => {
                                            // Got ICE Data, let's add the candidates to the WebRTC client
                                            ...
                                            client.getEventBus().on('connectionstate', (event) => {
                                                ...
                                                if (event.state === 'connected') {
                                                    ...
                                                    // Connection has been closed. We have to clean up here
                                                    console.log(':: We are disconnected!');
                                                }
                                            });
                                        }).catch((error) => {
                                            console.log('xCloudPlayer Client - ICE Exchange error:', error);
                                        });
                                    }).catch((error) => {
                ...
                reject({ error: 'Only xhome is supported as the type to start a new session' });
            }
        });
    },

    // Function to check if streaming sessions are ready
    isSessionsReady() {
        return new Promise((resolve, reject) => {
    ...
    // Function to check if exchange is ready
    isExchangeReady(url) {
        return new Promise((resolve, reject) => {
